...
var validator = require('validator'); // Оно идёт в папочку node_modules в файл index.js где благодаря export подтягивает объект в код
...
greeter('Hello my pussy friend! ヽ(°□° )ノ'); // В том файле, где я хочу поюзать экспортируемую сущность использую
...
console.log('Is Mangozedog.com a valid email?: ', validateEmail('Mangozedog.com')); // Теперь запуская наш скрипт в консоле через "npm start" - увидим результат работы,
